  ...
  useEffect(() => {
    ...
    function animate() {
      ...
      particles.forEach(particle => {
        ...
        ctx.beginPath();
        ctx.arc(particle.x, particle.y, particle.radius, 0, Math.PI * 12);
    ...
    <div className="relative">
      ...
      <div className={`min-h-screen flex items-center justify-center ${lato.className} relative z-10 pt-20`}>
        <div className="bg-gradient-to-b from-black via-black/70 to-black p-8 rounded-lg shadow-xl w-full max-w-md border border-gold-400">
          <h2 className={`text-4xl font-bold text-white mb-6 text-center ${playfair.className}`}>Welcome Back</h2>
          <p className="text-lg text-white mb-6 text-center">Sign in to access your exclusive jewelry collection</p>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <button
                type="submit"
                className="w-full flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-lg font-medium text-white bg-gold-400 hover:bg-gold-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gold-400 transition duration-300"
              >
                Sign In
              </button>
            </div>
          ...
          <p className="mt-6 text-center text-white text-lg">
            Don't have an account? <a href="/auth/register" className="text-gold-400 hover:underline">Register here</a>
          </p>
        </div>
      </div>
    </div>
